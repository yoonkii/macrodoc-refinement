...
export const DEFAULT_PROFILES: StyleProfile[] = [
  ...
  // ── Voice style pack (v1) — added as defaults 2026-07-12. Existing users
  // receive these via the style-profiles persist migration (version 1).
  {
    id: defaultId('personality'),
    name: "Detective Noir",
    type: 'personality',
...
/**
 * Names of the v1 voice-pack defaults. The style-profiles store's persist
 * migration merges exactly these (and only these) into pre-v1 persisted
 * state, so users who deleted an older default never see it resurrected.
 */
export const VOICE_PACK_V1_NAMES: string[] = ["Detective Noir", "Nature Documentary", "Movie Trailer", "Zen Master", "Passive-Aggressive Coworker", "Lumon Wellness", "K-Drama Subtitles", "Diplomatic Translator"];
