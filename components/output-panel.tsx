...
export function OutputPanel() {
  ...
  // Clear platform outputs when refined text changes (e.g. style/tone toggled)
  // so platform tabs will re-generate with the new style when clicked.
  const prevRefinedRef = useRef(store.refinedText);
  useEffect(() => {
    if (store.refinedText !== prevRefinedRef.current && store.refinedText.length > 0) {
      prevRefinedRef.current = store.refinedText;
      if (Object.keys(multiPostStore.platformOutputs).length > 0) {
        multiPostStore.clear();
        autoGeneratedRef.current.clear();
      }
    }
  }, [store.refinedText, multiPostStore]);

  // Clear platform outputs immediately when active style profiles change so
  // platform tabs regenerate with the new combined personality + platform styles.
  const prevProfileKeyRef = useRef(
    activeProfiles.map((p) => p.id).sort().join(",")
  );
  useEffect(() => {
    const profileKey = activeProfiles.map((p) => p.id).sort().join(",");
    if (profileKey !== prevProfileKeyRef.current) {
